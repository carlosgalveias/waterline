...
module.exports = function integrate(cache, joinInstructions, primaryKey, cb) {
  ...
  // Ensure valid usage
  var invalid = false;
  invalid = invalid || anchor(cache, { isObject: true });
  invalid = invalid || anchor(joinInstructions, { isArray: true });
  invalid = invalid || anchor(joinInstructions[0], { isObject: true });
  invalid = invalid || anchor(joinInstructions[0].parent, { isString: true });
  invalid = invalid || anchor(cache[joinInstructions[0].parent], { isObject: true });
