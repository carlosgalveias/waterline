  ...
  async.each(validations, function _eachValidation(validation, cb) {
    ...
    switch (curValidation.type) {
      ...
      case 'float':
        ...
        break;
      case 'password':
        rule = { isNotEmptyString: true };
        break;
      case 'json':
      case 'string':
        rule = curValidation.required ? { isNotEmptyString: true } : { isString: true };
        break;
      default:
    ...
    try {
      ...
      console.error(e)
      // Handle fatal error:
      console.log('validation error 221'); return cb(e);
    }
    ...
    if (!curValidation.required) {
      if (value === null || value === '') {
        console.log('validation true 235')
        return cb();
      }
    }
    ...
    if (curValidation.required && curValidation.type === 'boolean' && (typeof value !== 'undefined' && value !== null)) {
      if (value.toString() === 'true' || value.toString() === 'false') {
        console.log('validation true 243'); return cb();
      }
    }
    ...
    if (hasOwnProperty(self.validations[validation], 'type') && self.validations[validation].type === 'integer') {
      if (utils.matchMongoId(value)) {
        console.log('validation true 250')
        return cb();
      }
    }
    ...
    console.log('validation true 262')
    return cb();
    ...
    async.each(requirements, function _eachKey(key, next) {
        try {
          ...
          console.log('key', requirements[key])
        } catch (e) {
          return next(e);
        }
      },
      function afterwards(unexpectedErr) {
        ...
        validationError.forEach(function(obj) {
          ...
          errors[validation].push({ rule: obj.rule, message: obj.message , validationError: obj});
        });
    ...
    // Handle fatal error:
    if (err) {
      console.log('validation err 335'); return cb(err);
    }


    if (Object.keys(errors).length === 0) {
      console.log('validation true 340'); return cb();
    }
    console.log({ errors })
    console.log('validation error 343'); return cb(undefined, errors);
  });
