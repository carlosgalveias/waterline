...
Transformation.prototype.serialize = function(attributes, behavior) {
  var self = this;
  var values = _.cloneWith(attributes);
    ...
    Object.keys(obj).forEach(function(property) {
      ...
      if (behavior === 'schema') {
        if (hasOwnProperty(self._transformations, property)) {
          obj[self._transformations[property]] = _.cloneWith(obj[property]);
          delete obj[property];
        }
        return;
      }
      ...
      if (property === 'select' && _.isArray(obj[property])) {
        var arr = _.cloneWith(obj[property]);
        _.each(arr, function(prop) {
          if(_.has(self._transformations, prop)) {
            var idx = _.sortedIndexOf(obj[property], prop);
            if(idx > -1) {
